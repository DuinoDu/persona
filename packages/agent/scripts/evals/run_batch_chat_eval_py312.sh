...
if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <log_path> <status_path> <output_dir> [eval_args...]" >&2
  exit 1
fi
...
if [[ -z "$PYTHON_BIN" ]]; then
  echo "python binary not found in candidate paths" >&2
  exit 1
fi
...
mkdir -p "$(dirname "$LOG_PATH")" "$(dirname "$STATUS_PATH")" "$OUTPUT_DIR" "$REPO_ROOT/.cache/triton" "$REPO_ROOT/.cache/torchinductor" "$REPO_ROOT/.cache/torch_extensions" "$REPO_ROOT/.tmp"
cd "$REPO_ROOT"
...
SCRIPT_PATH="$REPO_ROOT/packages/agent/scripts/evals/batch_chat_eval_qwen35_9b.py"
CMD=("$PYTHON_BIN" -u "$SCRIPT_PATH" --output-dir "$OUTPUT_DIR" "$@")
