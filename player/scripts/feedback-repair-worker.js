...
function getAllowedSubtitleRoots() {
  ...
  const rawTextsDir =
    process.env.RAW_TEXTS_DIR || path.join(ROOT, "public", "audios", "raw_texts");
  addRoot(resolveTranscriptsDir(rawTextsDir));
  addRoot(process.env.RAW_TRANSCRIPTS_DIR);
  addRoot(process.env.PARTS_ROOT_DIR || path.join(ROOT, "..", "data", "03_parts"));
...
function resolveAudioPath(feedback) {
  ...
  const rawAudiosDir =
    process.env.RAW_AUDIOS_DIR || path.join(ROOT, "..", "data", "01_downloads");
  ...
  const directPublicPath = path.join(ROOT, "public", "audios", "single", filename);
  if (fs.existsSync(directPublicPath)) return directPublicPath;

  const publicAudiosDir = path.join(ROOT, "public", "audios");
...
function resolvePartAudioPath(feedback, rawAudiosDir) {
  ...
  const partsRoot = process.env.PARTS_ROOT_DIR || path.join(ROOT, "..", "data", "03_parts");
