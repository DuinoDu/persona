...
DB = '/home/duino/ws/ququ/process_youtube/ququplayer/prisma/dev.db'
CONV_DIR = '/home/duino/ws/ququ/process_youtube/ququplayer/public/audios/conversations'
...
for fn in sorted(os.listdir(CONV_DIR)):
    ...
    while new_name in used_names:
        ...
        idx += 1
    used_names.add(new_name)
    old_path = os.path.join(CONV_DIR, fn)
    new_path = os.path.join(CONV_DIR, new_name)
    try:
...
for old_name, new_name in rename_map.items():
    ...
    try:
        if new_date:
            c.execute('UPDATE Audio SET filename=?, filepath=?, date=? WHERE filename=?',
                      (new_name, new_path, new_date, old_name))
        else:
            c.execute('UPDATE Audio SET filename=?, filepath=? WHERE filename=?',
