...
import { buildArenaComparisons, loadGenerationRecords } from "@/lib/evalArtifacts";
...
export default async function EvalArenaPage({
  ...
  if (leftRun?.outputDir && rightRun?.outputDir) {
    const [leftRecords, rightRecords, judgments] = await Promise.all([
      loadGenerationRecords(leftRun.outputDir),
      loadGenerationRecords(rightRun.outputDir),
