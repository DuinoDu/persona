...
if [[ $# -lt 4 ]]; then
  echo "Usage: $0 <log_path> <status_path> <output_json> <checkpoint_dir> [eval_args...]" >&2
  exit 1
fi
...
mkdir -p "$(dirname "$LOG_PATH")" "$(dirname "$STATUS_PATH")" "$(dirname "$OUTPUT_JSON")" "$REPO_ROOT/.cache/triton" "$REPO_ROOT/.cache/torchinductor" "$REPO_ROOT/.cache/torch_extensions" "$REPO_ROOT/.tmp"
cd "$REPO_ROOT"
...
SCRIPT_PATH="$REPO_ROOT/scripts/sft/checkpoint_chat_eval_qwen35_9b.py"
CMD=("$PYTHON_BIN" -u "$SCRIPT_PATH" --checkpoint-dir "$CHECKPOINT_DIR" --output-json "$OUTPUT_JSON" "$@")
