#!/bin/bash
# Serve a local MP3 directory via Cloudflare Tunnel.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SERVE_DIR="${SERVE_DIR:-data/02_audio_splits}"
SERVE_PORT="${SERVE_PORT:-8890}"
...
if [ ! -d "$SCRIPT_DIR/$SERVE_DIR" ]; then
    echo "❌ Serve directory not found: $SCRIPT_DIR/$SERVE_DIR"
    exit 1
fi
...
cd "$SCRIPT_DIR/$SERVE_DIR"
python3 -m http.server "$SERVE_PORT" >"$HTTP_LOG" 2>&1 &
HTTP_PID=$!
echo "$HTTP_PID" > "$HTTP_PID_FILE"

echo "🌐 Starting Cloudflare Tunnel..."
"$HOME/.local/bin/cloudflared" tunnel --url "http://localhost:$SERVE_PORT" >"$TUNNEL_LOG" 2>&1 &
TUNNEL_PID=$!
...
echo "📁 Serving: $SCRIPT_DIR/$SERVE_DIR"
