...
SRC_MP3 = BASE / 'data/01_downloads/曲曲2025（全）/53 - 曲曲現場直播 2025年7月31日 ｜ 曲曲麥肯錫.mp3'
OUT_DIR = BASE / 'data/03_transcripts/曲曲2025（全）/53 - 曲曲現場直播 2025年7月31日 ｜ 曲曲麥肯錫_processed'
...
def measure_sentence(section: dict, sent: dict) -> dict:
    ...
    # Use ffmpeg volumedetect on a trimmed segment
    cmd = [
        ffmpeg_bin,
        '-hide_banner','-nostats',
        '-ss', f'{start:.2f}',
...
def process_section(section: dict) -> dict:
    ...
    # sample up to 30 sentences evenly to control runtime
    n = len(sentences)
    idxs = list(range(n))
    if n > 30:
        step = max(1, n // 30)
    ...
    # simple heuristic: if call段没有guest或两者均为空标，标记疑似错误
