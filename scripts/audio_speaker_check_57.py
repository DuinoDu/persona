...
SRC_MP3 = BASE / 'data/01_downloads/曲曲2025（全）/57 - 曲曲現場直播 2025年8月14日 ｜ 曲曲麥肯錫.mp3'
OUT_DIR = BASE / 'data/03_transcripts/曲曲2025（全）/57 - 曲曲現場直播 2025年8月14日 ｜ 曲曲麥肯錫'
MANIFEST = OUT_DIR / '_section_manifest.raw.json'
REPORT = OUT_DIR / '_speaker_feature_report.json'
SUMMARY = OUT_DIR / '_speaker_feature_summary.json'
...
# sample up to N sentences evenly to control runtime
SAMPLE_N = 36
...
def process_section(section: dict) -> dict:
    ...
    try:
        ...
        return {'title': title, 'ok': False, 'error': f'json parse error: {e}'}
...
summary = {
    ...
    'issues': [r for r in results if r.get('agg', {}).get('suspect')]
}
